...
function frontend() {
  ...
  urls=(
    ...
    nextjs         $(_frontend_fallback 'nextjs.org/docs/')
  ...
  if [[ $# -lt 2 ]]; then
    ...
    print -P ""
    print -P "Örnek kullanım: frontend npmjs mocha (veya sadece: npmjs mocha)."
    print -P ""
    return 1
  fi
  ...
  if [[ -z "$urls[$1]" ]]; then
    echo "Aranan \"$1\" bağlamı mevcutta desteklenmiyor."
    echo ""
    echo "Geçerli bağlamlar:"
    echo ""
    ...
    echo "  mdn, npmjs, nodejs, nextjs, qunit, reactjs, smacss, stackoverflow, unheap, vuejs, bundlephobia"
    echo ""
    return 1
  fi
